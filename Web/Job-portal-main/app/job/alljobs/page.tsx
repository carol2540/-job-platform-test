...
const AllJobs = () => {
  ...
  const uniqueLocations = useMemo(() => {
    return Array.from(new Set(JobData.map((job) => job.location))).sort();
  }, []);

  const uniqueTags = useMemo(() => {
    const tags = JobData.flatMap((job) => job.tags);
  ...
  }, [searchQuery, locationFilter, tagFilter]);
